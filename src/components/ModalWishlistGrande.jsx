...
export function ModalWishlistGrande({ item, onCerrar, onEditar }) {
  ...
  return (
    <div className="modal-overlay" onClick={onCerrar} style={{ backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', backgroundColor: 'rgba(0, 0, 0, 0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 10000 }}>
      <div className="animation-slide-up-fijo" onClick={(e) => e.stopPropagation()} style={{ width: '90%', maxWidth: '380px', maxHeight: '90dvh', overflowY: 'auto', backgroundColor: 'var(--color-superficie)', borderRadius: '28px', padding: '24px', display: 'flex', flexDirection: 'column', gap: '15px', boxShadow: 'var(--sombra-fuerte)', position: 'relative', boxSizing: 'border-box' }}>

        {/* Botón Editar (Izquierda) */}
        <button
          onClick={() => onEditar(item)}
          aria-label="Editar artículo"
          style={{ position: 'absolute', top: '18px', left: '18px', backgroundColor: 'var(--superficie-translucida)', backdropFilter: 'blur(10px)', border: 'none', color: 'var(--color-texto)', cursor: 'pointer', width: '38px', height: '38px', borderRadius: '50%', display: 'flex', justifyContent: 'center', alignItems: 'center', boxShadow: 'var(--sombra-suave)', zIndex: 10 }}
        >
        ...
        <button
          onClick={onCerrar}
          aria-label="Cerrar"
          style={{ position: 'absolute', top: '18px', right: '18px', backgroundColor: 'var(--superficie-translucida)', backdropFilter: 'blur(10px)', border: 'none', fontSize: '18px', fontWeight: 'bold', color: 'var(--color-texto)', cursor: 'pointer', width: '38px', height: '38px', borderRadius: '50%', display: 'flex', justifyContent: 'center', alignItems: 'center', boxShadow: 'var(--sombra-suave)', zIndex: 10 }}
        >✕</button>
        ...
        {item.link && (
          <a
            href={convertirAEnlaceAfiliado(item.link)}
            target="_blank"
